...
def main():
    ...
    BreadPerGeneration = 15

    #how close does a duck need to be to eat a bread
    minbreaddistance = 15

    #how much reward for eating a bread (max hunger == 1000 then he is not hungry anymore)
    breadeatreward = 300

    #the moving penalty (is muliplied with speed)
    penaltyformoving = 1.1

    #minium hunger to reproduce (you cant reproduce if you are almost dead)
    minhungertoreproduce = 100

    #how much time should be between each eovlution sequence
    timetoevolve = 500
    ...
    for i in range(ducksPerGeneration):
        Ducks.append(Duck(xpos = random.randint(minxy, maxx), ypos = random.randint(minxy, maxy), speed =(random.randint(2,5)), hunger = 1000, movement = random.randint(1,100)))
    ...
    def evolve():
        ...
        if len(duckspeedlist) >= 2:
            ...
            for gene in newspeedlist: #here we add a new duck to the game, so here more mutations can take place
                ...
                slowerorfaster = random.randint(-1,1) #mutation 
    ...
    def show_data():
        ...
        plt.plot(timecounterList, ducksandspeedlist,"ro-")
    ...
    while True:
        ...
        for i,Ducky in enumerate(Ducks):
            ...
            if Ducky.speed == 0:
                Ducky.hunger -= 100 #incase duck has 0 speed
        ...
        ducksandspeedlist.append((sum(duckspeedlist) / len(duckspeedlist)))
        ...
        if timecounter == timetoevolve:
            ...
            evolve()
            timecounter = 0 #reset time to evolve
